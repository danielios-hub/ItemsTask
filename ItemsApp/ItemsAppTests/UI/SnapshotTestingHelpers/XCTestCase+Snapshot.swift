...
extension XCTestCase {
    public func snapshotTest(snapshot: UIImage, named name: String, file: StaticString = #filePath, line: UInt = #line) {
        let key = URL(fileURLWithPath: String(describing: file)).lastPathComponent.components(separatedBy: ".").first
        XCTAssertNotNil(key, "Unable to extract snapshot key at file: \(file)", file: file, line: line)
        
        switch SnapshotTesting.strategy(for: key!) {
        case .record:
